...
Use ordinary focused live calls for routine play and packet/context only when deeper retrieval is needed; keep full evidence in raw/ and the journals. Replace the compact transfer checkpoint before compaction or stopping. Resume returns its next action and uncertainty, plus markers for later local changes. A resumed live connection must rebind identity; reconcile previous-session action outcomes explicitly. Never replay orders or reinterpret a stored checkpoint as proof of current safety. Use the persistent standard MCP connection in [control](control.md) for repeated play calls; no continuation-plan qualification is required.

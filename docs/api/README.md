...
Persistent session discovery serves a small stable surface: `rw_capabilities`, `rw_read`, `rw_act`, `rw_wait`, `rw_retrieve`, plus `rw_observe` and `rw_guard`. The113-tool upstream catalog stays reachable by name through `rw_read`/`rw_act`, with `rw_capabilities` supplying a compact overview/domain/workflow map—including trade—and exact schemas on request; it is no longer advertised in `tools/list`. `rw_read` is self-contained unless the caller explicitly supplies `delta:true` with `since`; usable bounded/partial facts remain under `data`. Trade-window reads include a bounded semantic trade view. `rw_wait` can return post-event context and verification in the same exchange, while `rw_observe` can materialize one selected decision packet. See [facade](../facade.md) for the surface and views, and [composition](../composition.md) for grouped-read and guarded-action examples.
...
[Observed response shapes](../../api/observed-shapes.json) are historical navigation evidence, not output schemas. Empty arrays and observed omissions cannot establish complete contracts. Runtime output preserves unfamiliar properties; inspect the current response and its coverage.

[Shared model behavior](MODELS.md) explains wrappers, event limits and visibility. These references help interpret an unfamiliar response but are not routine preload material or live availability proof.

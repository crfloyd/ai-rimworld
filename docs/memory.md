...
STRATEGY is one current account. Replace its contents at a handoff; never paste earlier strategy or handoff bodies below the current state. Immutable files under `handoffs/`, journals and evidence already preserve the superseded history. Keep current participant roles, dependencies, pending request/actual process handle, temporary settings and restoration conditions without contradictory CURRENT/emergency overlays. The live player owns current-state edits; reporting normally follows at a safe pause. Capture current state before compaction or transfer without an arbitrary limit that drops material current facts.

handoff captures rules, the current strategy version, evidence pointers, issues, optional tracked outcomes, local lessons, journal boundaries and controller/request state, including an unfinished composition and handle. Snapshots remain complete and immutable on disk. Default resume returns only handoff identity, next action/uncertainties, current-file pointers, change flags and a packet index; `--full-output` explicitly loads archived rules, strategy and packet detail.

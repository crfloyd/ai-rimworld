...
A section whose only problem is an upstream size guard or an upstream truncation flag is marked `degraded` with its retry advice, and the remaining queries still run. The response lists those keys under `degraded`; `stopped` still means everything after that point was abandoned for review. The decision preset's `world` topic now reads `list_world_objects` with `kind` defaulting to `caravans`, selectable with `world_kind`, so it never trips the guard on its own; the `visitors` topic reads neutral pawns on the current map.

...
Session discovery serves a small surface: `rw_read`, `rw_act`, `rw_wait`, `rw_capabilities`, `rw_retrieve`, `rw_observe` and `rw_guard`. The full upstream catalog stays reachable by name through `rw_read`/`rw_act`. On entering a run, pull the whole capability index once with `rw_capabilities` and no query: it returns every tool name and a one-line description in one call, so strategy is chosen against the real action space rather than the tools you happen to remember. Narrow with a query, and request a schema only for the tool actually chosen. Filter at the source, keep the default compact view, and escalate with `rw_retrieve` only when compact is genuinely insufficient. See [facade](docs/facade.md) for views, selection, limits and references.

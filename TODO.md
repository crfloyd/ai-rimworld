...
A/B trials and independent audits are complete (docs/PLAY-LOOP-EXPERIMENT.md).0.5.0 removes mandatory bulk-state preload and default per-call goal bookkeeping, and corrects HP-increase critical labels. Verify the ordinary path live, then prioritize useful gameplay/continuity measurement. Do not claim exact context-token savings or model reaction latency from wire bytes/chained-read timing. Host review and native MCP deployment remain separate unisolated factors.
